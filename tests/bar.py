...
def bar_problem_setup(stabilise=False,increments=1, force_direction=1, force_magnitude=-100, problem_description="florence bar"):
    ...
    fem_solver = FEMSolver(
        ...
        # === Debugging prints ===
        debug=True,
        debug_description="bar loadcase y",
...
def bar_MR(simulation_type="F", stabilise_tangents=True, problem_description="florence bar MR/florence bar MR"): # use description with break_newton_after for easy export
    ...
    # Read gmsh file, create boundary conditions and solver
    mesh, boundary_condition, fem_solver = bar_problem_setup(stabilise=stabilise_tangents,increments=1, force_direction=1, force_magnitude=100, problem_description=problem_description)
